...
const recipientEmail = async (req,request,template,subject,body) => {
  for(let index in request.recipients){
    let userLoginlog = await UserLoginLog.find({ user_id: request.recipients[index].recipient_user_id, thru_mobile: 1 }).sort('login_log_id DESC').limit(1);
...
module.exports = async (template,req) => {
  ...
  if(request.recipients && request.recipients.length > 0){
    ...
    await recipientEmail(req,request,template,subject,body);
  }else{
