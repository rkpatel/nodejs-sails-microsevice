...
const createString = async (bodyText,anniversaryNotiEmpgroup) => {

  for(const notiArr of anniversaryNotiEmpgroup){
    bodyText +=  '<tr>';
...
module.exports = async (template,req) => {
  ...
  if(!request.email_noti && request.anniversary.length <= 0) {return null;}
  ...
  for (const data of request.anniversary) {
    ...
    let bodyText = '';
    let anniversaryNotiEmpgroup = [];
    for(let i = 0; i < anniversaryNotiEmp.length; i += 3){
      anniversaryNotiEmpgroup.push(anniversaryNotiEmp.slice(i, i + 3));
    }
    if(anniversaryNotiEmp.length > 0){

      bodyText = await createString(bodyText,anniversaryNotiEmpgroup);
