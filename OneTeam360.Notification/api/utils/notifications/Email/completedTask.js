...
module.exports = async (template,req) => {
  ...
  const user = await Users.findOne({email: mailData.email});
  const employeedetails = await EmployeeProfile.findOne({employee_profile_id: mailData.employee_profile_id}).usingConnection(req.dynamic_connection);
