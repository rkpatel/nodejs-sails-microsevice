...
const authReuest = async (empProfile,res) => {
...
module.exports = async (req, res, next) => {
  ...
  { headers: { Authorization: req.headers.authorization } }
    ...
    if(response.data.status === RESPONSE_STATUS.success){
      ...
      const action = await RolePermission.find({ role_id: empProfile.role_id }).populate('permission_id').usingConnection(connection);

      let permissionList = [];
      let permissionModuleList = [];
      let isExposedApi;
      if(!req.headers['ocp-apim-subscription-key']){
        await authReuest(empProfile,res);
        isExposedApi = false;
        for (const permission in action) {
          let permissionModule = await PermissionModule.findOne({ permission_module_id: action[permission].permission_id.permission_module_id }).usingConnection(connection);
