...
const { ALLOWED_IMAGES,PROFILE_PIC_DIMENSIONS_IN_PIXELS } = require('../utils/constants/validations');
...
const validateImageUpload = function (fileArr,{ invalidFileTypeMsg ,fileAllowedTypes, maxUploadSize,tmpDir,requestFor}) {
  ...
  fileArr.forEach((fileData) => {
    ...
    const isImage = typeArr.some(type => ALLOWED_IMAGES.includes(type));
    if(isImage && requestFor === UPLOAD_REQ_FOR.profile_picture){
      const dimensions = sizeOf(fileData.fd);
      if(dimensions.width > PROFILE_PIC_DIMENSIONS_IN_PIXELS.WIDTH || dimensions.height > PROFILE_PIC_DIMENSIONS_IN_PIXELS.HEIGHT){
        dimensionError = true;
        const PROFILE_PIC_DIMENSION_ERROR = messages.PROFILE_PIC_DIMENSION_ERROR;
        dimensionErrMsg = PROFILE_PIC_DIMENSION_ERROR.replace(/STR_WIDTH/, PROFILE_PIC_DIMENSIONS_IN_PIXELS.WIDTH);
        dimensionErrMsg = dimensionErrMsg.replace(/STR_HEIGHT/, PROFILE_PIC_DIMENSIONS_IN_PIXELS.HEIGHT);
      }
    }
  ...
  if (sizeError || typeError || dimensionError) {
    let msgArr = [];
    if (typeErrMsg) { msgArr.push(typeErrMsg); }
    if (sizeErrMsg) { msgArr.push(sizeErrMsg); }
    if(dimensionErrMsg) { msgArr.push(dimensionErrMsg); }
...
module.exports = {
  uploadFile: async (upload, containerName, tmpDir, {
    invalidFileTypeMsg,
    maxUploadFileCount,
    maxUploadSize,
    fileAllowedTypes,
    ...
    try {
      return new Promise((resolve, reject) => {
        try {
          let maxBytes = localconfig.FILETYPE_MAXBYTES;
          ...
          upload.upload(uploadObject, async (err, files) => {
            ...
            if(files.length > MAX_UPLOAD_FILES_LIMIT){
              ...
              isValid = validateImageUpload(files,{invalidFileTypeMsg,fileAllowedTypes,maxUploadSize,tmpDir,requestFor});
            }
            if (isValid.status) {
