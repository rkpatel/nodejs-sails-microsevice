...
module.exports.cronjobs = {
  ...
  {
    var jsonArray = [];
    jsonArray.push({interval: process.env.NOTIFICATION_CRON_ADMIN,method: 'notificationCron'});
    return jsonArray;

  },
  notificationCron: async function(){
    if(process.env.NODE_ENV !== 'local' || process.env.ENV !== 'dev'){

      await require('../api/controllers/NotificationController').run();
    }
  },
